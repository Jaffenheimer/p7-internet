...
test("Renders the favoriteBox with correct title and back button, and correct text when no favorite recipes are added", () => {
    ...
    expect(screen.getByText(/Ingen opskrifter er blevet markeret som favorit./)).toBeInTheDocument();
  });

test("Clicking on the back button calls onclick function", () => {
    renderComponent(<FavoritesBox favoriteRecipes={[]} closeModal={pageActions.closeLoginModal}/>);
    const backCross = screen.getByTestId('CloseModalCross')
...
test("Selecting a recipe, calls onclick function without problems when the selected recipe is in the recipes object", () => {
  const recipes = [{
    ...
    renderComponent(<FavoritesBox favoriteRecipes={["test","2"]} recipes={recipes} closeModal={pageActions.closeLoginModal}/>);
    const buttons = screen.getAllByRole('button');
...
test("Selecting a recipe, makes toast appears when the selected recipe is not in the recipes object", async () => {
  const recipes = []
    renderComponent(<FavoritesBox favoriteRecipes={["1","2"]} recipes={recipes} closeModal={pageActions.closeLoginModal}/>);
    render(<ToastContainer 
      position="top-center"/>)
    const buttons = screen.getAllByRole('button');
    userEvent.click(buttons[0]);
    expect(await screen.findByText(/1 er ikke i listen af opskrifter på databasen. Prøv at vælge en anden opskrift./)).toBeInTheDocument();
  });

test("Clicking on the remove cross calls onclick function", () => {
    renderComponent(<FavoritesBox favoriteRecipes={["1","2"]} closeModal={pageActions.closeLoginModal}/>);
    const recipeCross = screen.getAllByTestId('RemoveFavoriteRecipeCross');
